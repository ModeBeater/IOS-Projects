...
import FirebaseAuth
class PartyRoom: UIViewController {
    ...
    func createPage(){
        ...
        let pcImage = fillWhiteImage(name: "Party")
...
//            pcImage.top
        ...
        let title = setLabel(string: "Party room", size: 20)
...
//        view.addSubview(firstRoom)
//        firstRoom.snp.makeConstraints{firstRoom in
//            firstRoom.width.equalTo(180)
//            firstRoom.height.equalTo(298)
//            firstRoom.top.equalTo(view.safeAreaLayoutGuide).offset(265)
//            firstRoom.leading.equalTo(view).offset(10)
//        }
//        let secondRoom = UIView()
//
//        // Add a border (stroke) to the rectangle view
//        secondRoom.layer.borderWidth = 2.0 // Set the border width as needed
//        secondRoom.layer.borderColor = UIColor.white.cgColor // Set the border color as needed
//
//        // Add the rectangle view to your main view
//        view.addSubview(secondRoom)
//        secondRoom.snp.makeConstraints{secondRoom in
//            secondRoom.width.equalTo(180)
//            secondRoom.height.equalTo(298)
//            secondRoom.top.equalTo(view.safeAreaLayoutGuide).offset(265)
//            secondRoom.leading.equalTo(view).offset(197)
//        }
...
//            make.leading.equalTo(view).offset(44)
        ...
        getNumberOfDevices { [self] numberOfDevices in
            for _ in 1...numberOfDevices{
                createComputers(x: Int(0.046 * screenWidth), y: Int(0.316 * screenHeight) * i, start1: i + 4 * (i - 1), end1: i + 1 + 4 * (i - 1), start2: i + 2 + 4 * (i - 1), end2: i + 4 + 4 * (i - 1))
                i += 1
                createComputers(x: Int(0.5 * screenHeight) , y: Int(0.316 * screenHeight) * i, start1: i + 4 * (i - 1), end1: i + 1 + 4 * (i - 1), start2: i + 2 + 4 * (i - 1), end2: i + 4 + 4 * (i - 1))
                i += 1
            }
            updateComputers()
            collectionView.reloadData()
        }
        let allComputersButton = UIButton()
        allComputersButton.setTitle("Забронировать всю комнату", for: .normal)
        allComputersButton.backgroundColor = UIColor(red: 0.484, green: 0.505, blue: 1, alpha: 1)
        allComputersButton.addTarget(self, action: #selector(allComputersSelected), for: .touchUpInside)
        allComputersButton.layer.cornerRadius = 12
        allComputersButton.titleLabel?.font = UIFont.systemFont(ofSize: 20)
        allComputersButton.layer.masksToBounds = true
        view.addSubview(allComputersButton)
        allComputersButton.snp.makeConstraints{button in
            button.width.equalTo(0.898 * screenWidth)
            button.height.equalTo(0.0578 * screenHeight)
            button.leading.equalTo(view).offset(0.02 * screenWidth)
            button.top.equalTo(view).offset(0.810 * screenHeight)
        }
    }
    func getNumberOfDevices(completion: @escaping (Int) -> Void) {
        var number = 1
        let documentReference = db.collection("Party").document("pcNumber")
...
//        view.addSubview(stackView)
    ...
    func updateComputers(){
        ...
        db.document("Party/pc").getDocument{ [self]snapshot, error in
...
//                    print("Naruto")
    ...
    @objc func allComputersSelected(_ sender: UIButton){
        var cnt = 0
        var block = false
        var tempButtons : [UIButton] = []
        for i in computerButtons{
            if block == false{
                if i.imageView?.tintColor != .red{
                    tempButtons.append(i)
                }
                else{
                    tempButtons = []
                    block = true
            ...
            if tempButtons.count == 10{
                tempButtons.forEach{button in
                    computerSelected(button)
                }
                return
            }
            cnt += 1
  ...
  //      dateFormatter.timeZone = TimeZone(identifier: "UTC")
    ...
    func updateTextHourPicker(){
        if getDay() == 1 || getDay() == 6{
            ...
            if selectedComputers.count == 10{
                dropDown.optionArray = [" 1 час - 12900 тенге", " 2 + 1 - 36000 тенге", " 3 + 2 - 38700 тенге"," день - 64500 тенге"," ночь - 64500 тенге"]
            }
        }
        else{
            ...
            if selectedComputers.count == 10{
                dropDown.optionArray = [" 1 час - 9900 тенге", " 2 + 1 - 26000 тенге", " 3 + 2 - 29700 тенге"," день - 49500 тенге"," ночь - 49500 тенге"]
            }
        }
...
//        payButton.lab = "оплатить"
    ...
    @objc func pay(_ sender: UIButton) {
        ...
        let standardPC = db.collection("Party").document("pc")
...
extension PartyRoom: UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
